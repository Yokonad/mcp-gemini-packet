...
export class PacketTracerLiveBridge {
  ...
  private isPacketTracerRunning(): boolean {
    const now = Date.now();
    if (now - this.packetTracerRunningLastCheckAt < PacketTracerLiveBridge.PROCESS_CHECK_INTERVAL_MS) {
  ...
  getStatus(): BridgeStatus {
    ...
    const pollingActive = ago !== null && ago * 1000 < PacketTracerLiveBridge.POLLING_ACTIVE_THRESHOLD_MS;
    ...
    const recentGrace = ago !== null && ago * 1000 < PacketTracerLiveBridge.CONNECTED_GRACE_MS;
...
<html lang="es">
  <head>
    ...
    <style>
      ...
      .sunken {
        ...
        border-bottom-color: #dfdfdf;
        border-right-color: #dfdfdf;
        box-shadow: inset 1px 1px #000, inset -1px -1px #fff, -1px -1px #dfdfdf, 1px 1px #808080;
      }
      ...
      .pager-btns button:active:not(:disabled) {
        border-top-color: #000;
        border-left-color: #000;
        border-bottom-color: #dfdfdf;
        border-right-color: #dfdfdf;
  ...
  <body>
    <div class="window">
      <div class="title-bar">
        ...
        <div class="title-controls">
          <div class="title-btn" style="line-height:8px">_</div>
          <div class="title-btn" style="font-size:14px;line-height:14px;padding-bottom:2px">□</div>
          <div class="title-btn" style="font-weight:900">X</div>
        </div>
      </div>
      ...
      <div class="window-body">
        ...
        <div class="table-container sunken">
          <table>
            <thead><tr><th>Timestamp</th><th>Kind</th><th>Detail</th></tr></thead>
            <tbody id="events"></tbody>
          </table>
        </div>
        
        <div class="pager">
          <div class="pager-info" id="pagerInfo">Page -- of --</div>
    ...
    <script>
      ...
      function eventClass(kind,detail){
        const s=(String(kind)+' '+String(detail)).toLowerCase();
        if(s.includes('error')||s.includes('fail')||s.includes('timeout')||s.includes('fallback')||s.includes('port-in-use'))return 'row-fail';
        if(s.includes('warning')||s.includes('warn')||s.includes('idle'))return 'row-warn';
        return 'row-ok'
      }
      ...
      function renderPage(){
        ...
        const rows=page.map(ev=>
          '<tr class="'+eventClass(ev.kind,ev.detail)+'"><td>'+esc(ev.ts)+'</td><td>'+esc(ev.kind)+'</td><td>'+esc(ev.detail)+'</td></tr>'
        ).join('');
        document.getElementById('events').innerHTML=rows;
        document.getElementById('pagerInfo').textContent='Page '+(currentPage+1)+' of '+totalPages;
      ...
      async function tick(){
        try{
          const sR=await fetch('/status',{cache:'no-store'});
          const s=await sR.json();
          setText('running',s.running,s.running?'ok':'bad');
          setText('connected',s.connected,s.connected?'ok':'bad');
          setText('pollingActive',s.polling_active,s.polling_active?'ok':'bad');
          setText('packetTracerRunning',s.packet_tracer_running,s.packet_tracer_running?'ok':'bad');
          setText('queueDepth',s.queueDepth??'-');
          setText('lastEvent',s.last_event??'-');
          setText('polls',s.polls??0);
          setText('queued',s.queued??0);
...
const bridgeSingleton = new PacketTracerLiveBridge();

export function ensureLiveBridgeStarted() {
  bridgeSingleton.start();
  return bridgeSingleton;
}

export function getLiveBridge() {
  return bridgeSingleton;
}
